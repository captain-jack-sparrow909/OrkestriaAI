...
test("protects the command center behind authenticated identity", async () => {
  for (const path of ["/dashboard", "/vela", "/loom", "/tempo", "/helio", "/aegis", "/enterprise", "/ecosystem", "/operations", "/pilot", "/scale", "/trust", "/cadence", "/ensemble", "/continuum", "/meridian", "/keystone", "/concord", "/verity", "/overture"]) {
    const response = await render(path);
...
test("ships the Phase 10 TrustGrid with truthful continuous trust and rollout gates", async () => {
  ...
  assert.match(chrome, /Phase 18 live/);
});
...
test("ships Phase 14 Meridian with evidence-gated portfolio intelligence", async () => {
  const [meridian, route, repository, orchestrator, schema, chrome] =
    await Promise.all([
      readFile(new URL("../app/meridian/MeridianStudio.tsx", import.meta.url), "utf8"),
      readFile(new URL("../app/api/meridian/route.ts", import.meta.url), "utf8"),
      readFile(new URL("../app/lib/platform/repository.ts", import.meta.url), "utf8"),
      readFile(new URL("../functions/orchestrator/src/main.js", import.meta.url), "utf8"),
      readFile(new URL("../appwrite/schema.mjs", import.meta.url), "utf8"),
      readFile(new URL("../app/components/WorkspaceChrome.tsx", import.meta.url), "utf8"),
  ...
  assert.match(meridian, /Choose the path/);
  assert.match(meridian, /Propose initiative/);
  assert.match(meridian, /Dependency map/);
  assert.match(meridian, /Update planning envelope/);
  assert.match(meridian, /Run portfolio scenario/);
  assert.match(meridian, /Investment decision gate/);
  assert.match(route, /propose_initiative/);
  assert.match(route, /run_scenario/);
  assert.match(route, /record_decision/);
  assert.match(repository, /planning_assumption/);
  assert.match(repository, /Investment cannot be authorized while evidence blockers remain/);
  assert.match(repository, /status: "recorded_no_commitment"/);
  assert.match(orchestrator, /path === "\/portfolio\/simulate"/);
  assert.match(orchestrator, /deterministic_portfolio_fixture/);
  assert.match(orchestrator, /financialCommitmentCreated: 0/);
  assert.match(orchestrator, /externalActionsExecuted: false/);
  assert.match(schema, /id: "strategic_goals"/);
  assert.match(schema, /id: "portfolio_initiatives"/);
  assert.match(schema, /id: "initiative_dependencies"/);
  assert.match(schema, /id: "capacity_envelopes"/);
  assert.match(schema, /id: "portfolio_scenarios"/);
...
test("ships Phase 16 Concord with bounded federated enterprise command", async () => {
  const [concord, route, repository, orchestrator, schema, chrome] =
    await Promise.all([
      readFile(new URL("../app/concord/ConcordCommand.tsx", import.meta.url), "utf8"),
      readFile(new URL("../app/api/concord/route.ts", import.meta.url), "utf8"),
      readFile(new URL("../app/lib/platform/repository.ts", import.meta.url), "utf8"),
      readFile(new URL("../functions/orchestrator/src/main.js", import.meta.url), "utf8"),
      readFile(new URL("../appwrite/schema.mjs", import.meta.url), "utf8"),
      readFile(new URL("../app/components/WorkspaceChrome.tsx", import.meta.url), "utf8"),
  ...
  assert.match(concord, /One enterprise/);
  assert.match(concord, /Workspace constellation/);
  assert.match(concord, /Authority ledger/);
  assert.match(concord, /Policy lattice/);
  assert.match(concord, /Enterprise operating rollup/);
  assert.match(concord, /Privacy-safe benchmark lens/);
  assert.match(concord, /Executive decision package/);
  assert.match(route, /propose_workspace/);
  assert.match(route, /propose_authority/);
  assert.match(route, /draft_policy/);
  assert.match(route, /run_rollup/);
  assert.match(route, /create_package/);
  assert.match(route, /decide_package/);
  assert.match(repository, /proposed_unverified_no_access/);
  assert.match(repository, /Executive package cannot be approved while evidence blockers remain/);
  assert.match(repository, /"approved_no_execution"/);
  assert.match(orchestrator, /path === "\/federation\/rollup"/);
  assert.match(orchestrator, /deterministic_federation_fixture/);
  assert.match(orchestrator, /otherMemberWorkspacesQueried: false/);
  assert.match(orchestrator, /rawTenantDataExposed: 0/);
  assert.match(schema, /id: "enterprise_federations"/);
  assert.match(schema, /id: "federation_workspaces"/);
  assert.match(schema, /id: "delegated_authorities"/);
  assert.match(schema, /id: "federated_policy_bindings"/);
  assert.match(schema, /id: "enterprise_rollups"/);
...
test("ships Phase 18 Overture with evidence-gated general availability command", async () => {
  const [overture, route, repository, orchestrator, schema, chrome] =
    await Promise.all([
      readFile(new URL("../app/overture/OvertureCommand.tsx", import.meta.url), "utf8"),
      readFile(new URL("../app/api/overture/route.ts", import.meta.url), "utf8"),
      readFile(new URL("../app/lib/platform/repository.ts", import.meta.url), "utf8"),
      readFile(new URL("../functions/orchestrator/src/main.js", import.meta.url), "utf8"),
      readFile(new URL("../appwrite/schema.mjs", import.meta.url), "utf8"),
      readFile(new URL("../app/components/WorkspaceChrome.tsx", import.meta.url), "utf8"),
  ...
  assert.match(overture, /The final movement/);
  assert.match(overture, /Launch readiness constellation/);
  assert.match(overture, /Load without mythology/);
  assert.match(overture, /Assurance without shortcuts/);
  assert.match(overture, /Connector certification board/);
  assert.match(overture, /Runbook library/);
  assert.match(overture, /Onboarding readiness/);
  assert.match(overture, /General availability decision room/);
  assert.match(route, /run_preflight/);
  assert.match(route, /propose_connector/);
  assert.match(route, /create_runbook/);
  assert.match(route, /create_onboarding/);
  assert.match(route, /request_launch/);
  assert.match(route, /decide_launch/);
  assert.match(repository, /internal_pre_ga/);
  assert.match(repository, /GA launch cannot be approved while evidence blockers remain/);
  assert.match(repository, /"approved_no_launch"/);
  assert.match(orchestrator, /path === "\/ga\/preflight"/);
  assert.match(orchestrator, /deterministic_load_fixture/);
  assert.match(orchestrator, /deterministic_security_fixture/);
  assert.match(orchestrator, /productionRequestsSent: 0/);
  assert.match(orchestrator, /externalPenTestCompleted: false/);
  assert.match(schema, /id: "ga_readiness_programs"/);
  assert.match(schema, /id: "load_test_runs"/);
  assert.match(schema, /id: "security_review_runs"/);
  assert.match(schema, /connectorCertifications/);
  assert.match(schema, /operationalRunbooks/);
  assert.match(schema, /onboardingChecklists/);
  assert.match(schema, /decisionStatus/);
  assert.match(chrome, /active === "overture"/);
  assert.match(chrome, /Phase 18 live/);
});
